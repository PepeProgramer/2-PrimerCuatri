...
int producir_dato(int value)
{
   int contador = producidos[value] + value * (num_items / num_prod);
   mtx.lock();
...
int ProdCons1SC::leer()
{
   ...
   while (primera_libre == 0) //if ( primera_libre == 0 )
      ocupadas.wait(); //ya no lleva guarda a diferencia de la SC
...
void ProdCons1SC::escribir(int valor)
{
   ...
   while (primera_libre == num_celdas_total) //if ( primera_libre == num_celdas_total )
      libres.wait();
...
void funcion_hebra_productora(MRef<ProdCons1SC>monitor, int k)
{
...
{
   cout << "-------------------------------------------------------------------------------" << endl
        << "Problema de los productores-consumidores (multiple prod/cons, Monitor SC, buffer FIFO). " << endl
        << "-------------------------------------------------------------------------------" << endl
        << flush;

   MRef<ProdCons1SC> mref = Create<ProdCons1SC>();
   ...
   for (int i = 0; i < num_prod; i++)
   {
      hebra_productora[i] = thread(funcion_hebra_productora, mref, i);
   }
   thread hebra_consumidora[num_cons];
   for (int i = 0; i < num_cons; i++)
   {
      hebra_consumidora[i] = thread(funcion_hebra_consumidora, mref);
   }
